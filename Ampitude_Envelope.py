...
class EnvelopeGenerate:
    def __init__(self, window_len:int=2048, hop_len:int=1024, num_bits:int=8) -> None:
    ...
    def normalize_env(self, envelope):
        ...
        normalized_envelope = ((envelope - min_val) / (max_val - min_val)) * (2 ** self.num_bits - 1)
        return normalized_envelope.astype(int)
    
    def save2file(self, envelope, filename):
        if np.issubdtype(envelope.dtype, np.integer):
            binary_strings = [format(int(val), '0{}b'.format(self.num_bits)) for val in envelope]
